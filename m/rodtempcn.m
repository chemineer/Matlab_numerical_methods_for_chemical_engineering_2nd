function z = rodtempcn(f,g0,g1,xf,tf,n,m,a)
% 단위길이 막대의 1차원 온도분포(Crank-Nicolson 방법)
% inputs:
...
d = (1 + r)*ones(1,n-1); av = -(r/2)*ones(1,n-2); av(n-1) = 0;
b = -(r/2)*ones(1,n-1); b(1) = 0; bv(1) = 0; dv(1) = d(1);
for i = 2:n-1, bv(i) = b(i)/dv(i-1); dv(i) = d(i) - bv(i)*av(i-1); end
for j = 1:m
    cv(1) = (r/2)*u(1,j+1) + (r/2)*u(1,j) + (1-r)*u(2,j) + (r/2)*u(3,j);
    for i = 2:n-2, cv(i) = (r/2)*u(i,j) + (1-r)*u(i+1,j) + (r/2)*u(i+2,j); end
    cv(n-1) = (r/2)*u(n-1,j)+(1-r)*u(n,j)+(r/2)*u(n+1,j)+(r/2)*u(n+1,j+1);
    zv(1) = cv(1);
